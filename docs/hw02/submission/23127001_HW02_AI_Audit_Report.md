...
| Artifact #4 - Bug tracker và evidence | Tool: Codex/ChatGPT. Prompt chính: chạy Playwright, phân tích fail, cập nhật trạng thái bug và ảnh minh chứng. | `bug-tracker.md`, `test-execution-notes.md`, `github-issues-draft.md`, `docs/hw02/evidence/*/screenshot.png`. | VALID | Kết quả được đối chiếu với Playwright run thật: 26 tests, 10 passed, 16 failed. Mỗi bug confirmed có screenshot. | Em xác nhận 16 bug từ report, đánh dấu Rejected cho candidate không tái hiện, copy screenshot vào thư mục evidence theo từng Bug ID. |
